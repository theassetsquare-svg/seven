...
> *"index.html, style.css, og.png, favicon, llms.txt, sitemap.xml 모두에서 도메인 `seven-97b.pages.dev`를 `<새도메인>` 으로, 전화번호 ``을 `<새번호>` 으로, 매장명·닉네임 `대전세븐나이트 / 원숭이`을 `<새매장 / 새닉>` 으로 바꿔서 푸시해줘. og.png과 favicon은 새 정보로 다시 그려줘."*
